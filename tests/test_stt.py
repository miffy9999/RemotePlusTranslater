...
def test_complete_flat_bundled_model_is_preferred(tmp_path):
    model = tmp_path / "small"
    model.mkdir()
    for name in ("config.json", "model.bin", "tokenizer.json", "vocabulary.txt"):
        (model / name).write_bytes(b"model")
    assert prepared_model_path(tmp_path, "small") == model


def test_incomplete_flat_bundled_model_is_rejected(tmp_path):
    model = tmp_path / "small"
    model.mkdir()
    (model / "config.json").write_text("{}", encoding="utf-8")
    assert prepared_model_path(tmp_path, "small") is None


def test_prepared_cache_snapshot_is_found(tmp_path):
    cache = tmp_path / "models--Systran--faster-whisper-small"
    ...
    assert prepared_model_path(tmp_path, "small") == snapshot
