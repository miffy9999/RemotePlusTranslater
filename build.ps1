...
# Internal design and QA documents stay in the source repository. The hotel
# package receives only the finished operator manual selected below.
$manualPdfs = @(
...
function Sign-CommercialArtifact([string]$Path) {
    ...
    $certificateEkus = @(
        $certificate.EnhancedKeyUsageList | ForEach-Object { [string]$_.ObjectId.Value }
    )
    if (-not ($certificateEkus -contains $codeSigningEku)) {
...
$sttModel = (& '.\.venv\Scripts\python.exe' -c 'from translator_app.config import load_config; print(load_config().stt.model)').Trim()
if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($sttModel)) {
    throw 'Could not determine the configured speech model.'
}
$whisperCache = ".\models\whisper\models--Systran--faster-whisper-$sttModel"
$whisperRef = Join-Path $whisperCache 'refs\main'
if (-not (Test-Path -LiteralPath $whisperRef)) {
    throw "Prepared Whisper model was not found: $whisperRef"
}
$whisperRevision = (Get-Content -LiteralPath $whisperRef -Raw).Trim()
$whisperSource = Join-Path $whisperCache "snapshots\$whisperRevision"
$whisperRequiredFiles = @('config.json', 'model.bin', 'tokenizer.json', 'vocabulary.txt')
foreach ($requiredFile in $whisperRequiredFiles) {
    if (-not (Test-Path -LiteralPath (Join-Path $whisperSource $requiredFile))) {
        throw "Prepared Whisper model is incomplete: $requiredFile"
    }
}
$whisperTarget = ".\dist\RemotePlusTranslator\models\whisper\$sttModel"
New-Item -ItemType Directory $whisperTarget -Force | Out-Null
foreach ($requiredFile in $whisperRequiredFiles) {
    Copy-Item -LiteralPath (Join-Path $whisperSource $requiredFile) `
        -Destination (Join-Path $whisperTarget $requiredFile) -Force
}
...
Get-ChildItem '.\dist\RemotePlusTranslator\models\hymt2\llama' -File -Filter '*.exe' |
    ...
    Remove-Item -Force
# Text-only llama-server does not load implementation DLLs belonging to the
# removed CLI/benchmark tools. Keep server implementation and shared CPU DLLs.
Get-ChildItem '.\dist\RemotePlusTranslator\models\hymt2\llama' -File -Filter '*-impl.dll' |
    Where-Object Name -ne 'llama-server-impl.dll' |
    Remove-Item -Force
