...
def doctor() -> int:
    ...
    try:
        ...
        from .stt import prepared_model_path

        speech_model = prepared_model_path(cfg.root / "models" / "whisper", cfg.stt.model)
        checks.append(
            (
                "Whisper model",
                speech_model is not None,
                str(speech_model or cfg.root / "models" / "whisper" / cfg.stt.model),
            )
        )
