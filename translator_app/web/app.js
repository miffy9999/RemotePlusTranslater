...
const cards = new Map();
let ui = localStorage.getItem('remoteplus-ui-language') || 'ja';
...
function t(key) { return (messages[ui] || messages.ja)[key] || messages.ja[key] || key; }
...
async function loadInitial(){
  const data=await api('/api/state'); languages=data.languages||[]; const select=$('#language'); select.replaceChildren();
  ...
  (data.history||[]).forEach(event=>handleEvent(event)); applyState(data.state||{});
  const devices=await api('/api/devices'); const input=$('#input-device'); input.replaceChildren(); const def=document.createElement('option');def.value='default';def.textContent=t('systemDefault');input.appendChild(def);
  (devices.inputs||[]).filter(x=>x.id!=='default').forEach(item=>{const option=document.createElement('option');option.value=item.id;option.textContent=item.name;input.appendChild(option)}); input.value=String(state.input_device||'default'); if(devices.warnings?.length)toast(devices.warnings[0]);
}
...
$('#ui-language').onchange=e=>{ui=e.target.value;localStorage.setItem('remoteplus-ui-language',ui);setTranslations()};
