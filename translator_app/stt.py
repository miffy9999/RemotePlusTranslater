...
# state and cache locks. Serialize construction if tests or maintenance tools
# create more than one recognizer in the same process.
_MODEL_LOAD_LOCK = threading.Lock()
REQUIRED_MODEL_FILES = ("config.json", "model.bin", "tokenizer.json", "vocabulary.txt")


def prepared_model_path(root: Path, model_name: str) -> Path | None:
    """Return a complete flat bundle or prepared Hugging Face cache snapshot."""
    configured = Path(model_name)
    candidates = [configured] if configured.is_absolute() else [root / model_name]
    for candidate in candidates:
        if candidate.is_dir() and all((candidate / name).is_file() for name in REQUIRED_MODEL_FILES):
            return candidate
    cache = root / f"models--Systran--faster-whisper-{model_name}"
    try:
        revision = (cache / "refs" / "main").read_text(encoding="utf-8").strip()
    except OSError:
        return None
    snapshot = cache / "snapshots" / revision
    if revision and all((snapshot / name).is_file() for name in REQUIRED_MODEL_FILES):
        return snapshot
    return None
...
class WhisperRecognizer:
    ...
    def load(self) -> None:
        ...
        with _MODEL_LOAD_LOCK:
            ...
            root_path = self.cfg.root / "models" / "whisper"
            root = str(root_path)
            local_model = prepared_model_path(root_path, self.model_name)
            model_source = str(local_model) if local_model is not None else self.model_name
            try:
