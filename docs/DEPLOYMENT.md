...
대상 PC에는 Python이나 Windows TTS 언어팩이 필요 없다. Chrome 또는 Edge가 있으면 독립 app 창으로 열고, 없으면 기본 브라우저를 사용한다. 인터넷은 최초 STT·번역·검증된 TTS 모델팩 설치와 업데이트에만 필요하며 통화 처리 중에는 필요하지 않다.
