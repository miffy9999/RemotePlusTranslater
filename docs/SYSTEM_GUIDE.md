...
- `sherpa-onnx`: 검증된 로컬 ONNX 음성 합성, pygame 재생
...
TTS는 검증된 Supertonic/Kokoro 팩으로 임시 WAV를 생성하고 pygame mixer가 선택된 SDL output으로 재생한다. 네이티브 합성은 별도 `RemotePlusTtsWorker` 프로세스에 격리한다. 새 요청은 queue 1의 대기 요청, 실행 중 worker 프로세스, 현재 playback을 중단한다. 유휴 worker는 모델을 메모리에 유지해 재사용한다. 설치되지 않은 언어는 텍스트만 표시하며 온라인 서비스로 우회하지 않는다.
