...
기존 고객·직원 혼합 WAV
  ...
  → 구간당 자동 언어 판별 1회 + 문자 체계 보정
  → 고객(추정) / 직원(추정) / 화자 미확인 카드
  → 시간순 원문·번역 + 해당 구간 원음 재생

번역 카드의 운영자 승인 교정
  → 원문 + 원문 언어 + 번역 언어로 로컬 번역 메모리 저장
  → 다음 동일 문장은 AI 호출 전에 즉시 교정본 반환
```

TTS, 출력 장치, 음성팩은 사용하지 않으며 직원은 번역문의 읽기 표기를 보고 직접 말한다.
WAV 자동 언어 판별은 강제 언어 재디코딩을 추가하지 않는다. 짧은 `check-out`, `credit card`처럼
Whisper 언어 ID와 실제 문자 체계가 충돌하는 경우에만 이미 생성된 전사문의 문자 체계로 언어를
보정하므로 STT 호출 수와 지연은 늘지 않는다.
