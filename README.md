...
`build.ps1`은 web/config, faster-whisper/ctranslate2, sounddevice/SoundCard, edge-tts/pygame와 DLL을 수집하고 Whisper·Hy-MT2·llama runtime을 복사한 뒤 완성 EXE로 doctor를 실행합니다. 웹 자산은 사용자 경로를 받는 StaticFiles 대신 세 개의 고정 allowlist 경로로만 제공합니다. 모델 경로는 배포 폴더 기준 상대 경로라 USB 복사 후에도 유지됩니다.
