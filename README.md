...
Python이 없는 다른 Windows PC에서도 `dist\RemotePlusTranslator` 폴더 전체를 복사하면 실행할 수 있습니다. STT·번역·TTS는 모두 PC 안에서 처리되며 사용량 과금이 없습니다. 인터넷은 최초 모델팩 설치와 업데이트에만 필요하고, 통화 중 고객 음성이나 직원 답변은 외부 TTS 서비스로 전송되지 않습니다.
...
4. 첫 실행이면 검증된 로컬 음성팩 전체를 백그라운드에서 자동으로 한 번 설치합니다. 설치가
   끝날 때까지 텍스트 번역은 사용할 수 있고, 이후 실행에서는 다시 다운로드하지 않습니다.
...
새 고객 통화를 시작할 때 `기록 지우기`를 누르면 메모리 대화 기록과 합성음성 고지 상태가
초기화됩니다. 다음 첫 자동 답변에는 고객 언어로 AI 번역·합성음성 사용 안내가 붙습니다.
...
`prepare_models.bat`으로 해당 언어의 검증된 로컬 음성팩을 설치했는지, TTS 토글과 출력 장치를 확인합니다. `/api/tts`의 `installed_languages`에 언어가 없으면 번역 텍스트는 표시하지만 음성은 외부 서비스로 우회하지 않습니다. 새 답변이 들어오면 이전 로컬 합성과 재생을 중단하고, 실패한 답변은 카드의 다시 듣기로 다시 생성할 수 있습니다.
